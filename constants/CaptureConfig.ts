...
export const CAPTURE_CONFIG = {
    ...
    // Row 2 (Bottom):   3 photos at pitch -50° — lower hemisphere (less visible)
    // Row 3 (Zenith):   1 photo  at pitch +85° — ceiling
    ROWS: 4,
    COLS_PER_ROW: [8, 6, 3, 1],
    ROW_PITCHES: [0, 50, -50, 85],
    HORIZONTAL_STEP: 45, // 360 / 8 = 45°
    TOTAL_PHOTOS: 18,
    ...
    // Generous tolerance for easy alignment
    ...
    ROW_ICONS: ['panorama-horizontal', 'arrow-upward', 'arrow-downward', 'vertical-align-top'] as const,
...
export function generateCapturePositions(): CapturePosition[] {
    ...
    for (let row = 0; row < CAPTURE_CONFIG.ROWS; row++) {
        ...
        for (let col = 0; col < colCount; col++) {
            // Stagger non-horizon rows for better coverage
            const yawOffset = (row === 1 || row === 2) ? yawStep / 2 : 0;
            positions.push({
                id: id++,
                row,
                col,
